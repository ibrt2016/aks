...
resource "azurerm_user_assigned_identity" "agic_user_assigned_identity" {
  count               = var.workload_identity_enabled && var.agic_enabled ? 1 : 0
  location            = var.location
  name                = var.agic_identity_custom_name
  resource_group_name = coalesce(var.aks_user_assigned_identity_resource_group_name, var.resource_group_name)
